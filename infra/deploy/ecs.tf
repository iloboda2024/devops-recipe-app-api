...
resource "aws_security_group" "ecs_service" {
  ...
  # HTTP inbound access for proxy app
  ingress {
    from_port   = 8000
    to_port     = 8000
    protocol    = "tcp"
...
resource "aws_ecs_service" "api" {
  ...
  # number of running instances in our service
  # if you add more it will increase cost but it gives the ability to process more requests and scale up your app
  desired_count    = 1
  launch_type      = "FARGATE"
  platform_version = "1.4.0"
  # allow for exec command
  enable_execute_command = true

  network_configuration {
    # for tests only, connect to from public
